...
public class HeroEpic : MonoBehaviour
{
    ...
    public int scoreCommon; //сила удара по жизн€м игрока
    ...
    private void DeathEpicHero()
    {
        ...
        Dead1(); //”брать когда будет много эпиков
        PlayAudioDeathG.SetActive(true);
        colorCouldron.DamagePlayerHP(scoreCommon);
        Debug.Log("ќбычный герой повержен!");
        //Destroy(gameObject, 0.30f); // или отключить скрипт геро€
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Enemy")
        {
            _anim.SetTrigger("Hit");
            //speed = 0;
            Debug.Log("Ёпик в боевое столкновение с врагом!");

        }
        //else
        //{
           // Debug.Log("Ёпик не в бою");
    ...
    private void OnTriggerEnter(Collider collision)
    {
        ...
        {
            Debug.Log("ѕопали дистанционной атакой!");
